...
public class TiledMapPhysicsLoader {
	...
	/**
	 * Create a new MapBodyManager instance.
	 * 
	 * @param world
		...
		try {
			...
			while (materialIt.hasNext()) {
				...
				if (!materialValue.has("name")) {
					Gdx.app.error(getClass().getSimpleName(), "material without name");
					continue;
				}
			...
			Gdx.app.error(getClass().getSimpleName(), "error loading " + materialsFile.name() + " " + e.getMessage());
		}
	}
